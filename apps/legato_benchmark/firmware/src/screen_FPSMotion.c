...
static void fpsUpdateTimer_Callback()
{
    ...
    //Update Refresh Rate
    leRenderer_DisplayInterface()->ioctl(GFX_IOCTL_GET_VSYNC_COUNT, &ioctlArg);
...
static void moveWidget(MOTION_WIDGET_IDX id)
{
    ...
    gfxDriverInterface.ioctl(GFX_IOCTL_GET_LAYER_RECT , &layerRect);
    rect.width = layerRect.width;
    rect.height = layerRect.height;
...
{
    uint32_t i;
    gfxIOCTLArg_DisplaySize displaySize;
    ...
    if (rectSize == FULLSCREEN_RECT_SIZE)
    {
        gfxDriverInterface.ioctl(GFX_IOCTL_GET_DISPLAY_SIZE, &displaySize);
        
        motionWidgets[RECT_WIDGET_0].widget->fn->setWidth(motionWidgets[RECT_WIDGET_0].widget, displaySize.width);
        motionWidgets[RECT_WIDGET_0].widget->fn->setHeight(motionWidgets[RECT_WIDGET_0].widget, displaySize.height);
