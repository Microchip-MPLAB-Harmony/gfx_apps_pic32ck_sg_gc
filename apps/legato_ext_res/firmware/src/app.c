...
void APP_ProcessSwitchPress(void)
{
    ...
    if(SW_1_STATE_PRESSED == (SW_1_Get()))
    {
...
void APP_Tasks ( void )
{
    ...
    switch(appData.state)
    {
        ...
        {
            DCACHE_INVALIDATE_BY_ADDR((uint32_t *)qspiData.readBuffer, 4);
            APP_ReadJedecId((uint32_t *)qspiData.readBuffer);
            qspiData.jedec_id = *((uint32_t*)qspiData.readBuffer);
        ...
        case APP_STATE_QSPI_WRITE:
        {
            ...
            DCACHE_CLEAN_BY_ADDR(cdcReadAddress, PAGE_SIZE);            
        ...
        {            
            ...
            DCACHE_CLEAN_BY_ADDR(sqiReadBuffAddress, PAGE_SIZE);
